...
function exibirDiagnosticoPreliminar(scores) {
  const gatilhoDominante = Object.keys(scores).reduce((a, b) =>
  ...
  const detalhesGatilhos = {
    Injustiça: {
      icon: "⚖️",
      description:
        "Você se sente mais reativo quando percebe que regras importantes foram quebradas ou que há falta de equidade.",
    },
    Frustração: {
      icon: "😤",
      description:
        "Sua reatividade aumenta quando obstáculos e ineficiências te impedem de progredir.",
    },
    Cansaço: {
      icon: "🔋",
      description:
        "Sua paciência se esgota quando suas reservas de energia física e mental estão no limite.",
    },
    "Medo de Julgamento": {
      icon: "👀",
      description:
        "Você se torna mais reativo em situações de exposição, onde se sente avaliado ou em risco.",
    },
  };
...
export default function FerramentaIrritacao() {
  ...
  useEffect(() => {
    const observer = new IntersectionObserver(
      ...
      { threshold: 0.1 }
    ); // Ativa quando 10% do elemento estiver visível
      ...
      function inicializarFerramenta() {
        ...
        function renderQuestionsAsCarousel() {
          ...
          questions.forEach((q, index) => {
            ...
            card.innerHTML = `<p class="${styles.questionText}"><b>${
              index + 1
            }.</b> ${q.text}</p><div class="${
              styles.sliderContainer
            }" data-question-id="${
              q.id
            }"><input type="range" min="0" max="4" value="0" class="${
              styles.irritationSlider
            }"><div class="${
              styles.sliderFeedback
            }">Nível de Incômodo: <span class="${
              styles.sliderValue
            }">0</span><span class="${styles.sliderLegend}">${
              sliderLegends[0]
            }</span></div></div>`;
        ...
        function updateCarousel() {
          ...
          carouselStage.style.transform = `translateX(${
            offset / questions.length
          }%)`;
          counterEl.textContent = `${currentQuestionIndex + 1} / ${
            questions.length
          }`;
        ...
        document.body.addEventListener("input", (e) => {
          if (e.target.matches(`.${styles.irritationSlider}`)) {
            ...
            const feedbackContainer = slider
              .closest(`.${styles.sliderContainer}`)
              .querySelector(`.${styles.sliderFeedback}`);
        ...
        document
          .querySelectorAll('[id^="start-irritation-btn"]')
          .forEach((button) => {
        ...
        document
          ...
          ?.addEventListener("click", async () => {
            ...
            if (!reflection1 || !reflection2 || !reflection3) {
              alert(
                "Por favor, preencha todas as perguntas de reflexão para continuar."
              );
              return;
            }
            const respostas = {};
            document
              .querySelectorAll(`.${styles.irritationSlider}`)
            ...
            scores = {
                ...
                'Medo de Julgamento': (r.q4 || 0) + (r.q8 || 0) + (r.q10 || 0),
            };
        ...
        document
          .getElementById("send-report-btn")
          ?.addEventListener("click", async (e) => {
            ...
            const finalPayload = {
              ...
              origem: "MapaDaIrritacao",
              scores: {
                medo:
                  (userResponses.respostas.q4 || 0) +
                  (userResponses.respostas.q8 || 0) +
                  (userResponses.respostas.q10 || 0),
                frustracao:
                  (userResponses.respostas.q2 || 0) +
                  (userResponses.respostas.q6 || 0) +
                  (userResponses.respostas.q9 || 0),
                injustica:
                  (userResponses.respostas.q1 || 0) +
                  (userResponses.respostas.q5 || 0) +
                  (userResponses.respostas.q11 || 0),
                cansaco:
                  (userResponses.respostas.q3 || 0) +
                  (userResponses.respostas.q7 || 0) +
                  (userResponses.respostas.q12 || 0),
                total: Object.values(userResponses.respostas).reduce(
                  (a, b) => a + b,
                  0
                ),
              },
              topGatilho: Object.keys(scores).reduce((a, b) =>
                scores[a] > scores[b] ? a : b
              ),
              respostas: userResponses.respostas,
              ...
              reflexao_nota: parseInt(
                document.getElementById("reflection-scale").value,
                10
              ),
            };
            try {
              ...
              if (!response.ok) {
                ...
                throw new Error(
                  errorData.message ||
                    "O servidor não pôde processar a solicitação."
                );
              }
  ...
  return (
    <>
      <Modals />
      {/* Seção de Introdução (agora é uma única seção com rolagem interna) */}
      <section id="step0" className="step active">
        <div className={styles.irritationPageWrapper}>
          {/* Bloco 1: Hero */}
          <div className={`${styles.fullScreenBlock} ${styles.firstBlock}`}>
            <div className={styles.heroSection}>
              <div className={styles.heroOverlay}></div>
              <div className={styles.heroContent}>
                <h1 className={styles.heroTitle}>
                  Você não se irrita por causa das situações.
                </h1>
                <div className={styles.heroBottomRow}>
                  <p className={styles.heroParagraph}>
                    Na prática, o que nos tira do controle quase nunca é o que
                    acontece...{" "}
                    <strong>é o gatilho interno que aquilo ativa.</strong>
                  </p>
                  <div className={styles.heroCtaContainer}>
                    <button
                      id="start-irritation-btn-hero"
                      className={styles.heroCtaButton}
                    >
                      Descobrir Meu Gatilho Agora
                    </button>
                  </div>
                </div>
              </div>
          ...
          {/* Bloco 2: Dor Silenciosa */}
          <div className={`${styles.fullScreenBlock} ${styles.calmBlock}`}>
            <div className={styles.introContainer}>
              <div className={styles.painPointSection}>
                <h2>Isso te soa familiar?</h2>
                ...
                <p className={styles.reactionExplosion}>
                  De repente, você reage de um jeito que nem você entende.
                </p>
                <p className={styles.reactionRegret}>
                  Depois vem a culpa, o arrependimento ou o silêncio.
                </p>
                <div className={styles.ctaSection}>
                  <button
                    id="start-irritation-btn-2"
                    className={styles.actionButton}
                  >
                    Mapear Minha Reação Agora
                  </button>
                </div>
              </div>
            </div>
          ...
          <div className={styles.fullScreenBlock}>
            <div className={styles.revelationContainer}>
              <h2 className={styles.revelationTitle}>
                A irritação não surge do nada. Ela segue um padrão.
              </h2>
              <div className={styles.revelationImage}></div>
              <div className={styles.revelationText}>
                <div className={styles.revelationTextContent}>
                  <p>
                    Cada pessoa tem um <strong>gatilho dominante</strong> que
                    acende a raiva mais rápido do que os outros. Conheça os 4
                    principais:
                  </p>
                  ...
                  <div className={styles.ctaSection}>
                    <button
                      id="start-irritation-btn-3"
                      className={styles.actionButton}
                    >
                      Descobrir Meu Padrão de Irritação
                    </button>
                  </div>
                </div>
              </div>
          ...
          {/* Bloco 4: CTA Final */}
          <div className={`${styles.fullScreenBlock} ${styles.finalBlockBg}`}>
            <div className={styles.introContainer}>
              <div className={styles.whyNowSection}>
                <h3>
                  Esse teste não é sobre te rotular.
                  <br />É sobre entender o ponto exato onde você perde o
                  controle.
                </h3>
                <p className={styles.goldSentence}>
                  "A raiva não é o problema.
                  <br /> O problema é não saber de onde ela vem."
                </p>
                <p>
                  Sem esse mapa, você continua reagindo no automático. Com o
                  mapa, <strong>você age antes da explosão.</strong>
                </p>
              </div>
              <div className={styles.ctaSection}>
                <button
                  id="start-irritation-btn-4"
      ...
      {/* As seções do teste (step1, 2, 3) ficam separadas */}
      <section id="step1" className={`step ${styles.testSection}`}>
        <div className={styles.testContainer}>
          <h2>Responda às 12 situações abaixo:</h2>
          <div className={styles.instructionsBox}>
            Para cada situação, use o controle deslizante para indicar seu nível
            de incômodo, de 0 (nenhum) a 4 (máximo).
          </div>
          ...
          <div id="carousel-navigation" className={styles.carouselNavigation}>
            <button id="carousel-prev" className="secondary-btn" disabled>
              &larr; Anterior
            </button>
            <span id="carousel-counter">1 / 12</span>
            <button id="carousel-next" className="secondary-btn">
              Próximo &rarr;
            </button>
          </div>
          <button id="goto-step2-btn" className="primary-btn hidden">
            Enviar Respostas e Continuar
          </button>
        </div>
      </section>

      <section id="step2" className={`step ${styles.testSection}`}>
        <div className={styles.testContainer}>
          <div className={styles.congratsMessage}>
            <h2>🎉 Parabéns! 🎉</h2>
            <h3>
              Sua Autoavaliação foi <b>🥇 Concluída. 🥇</b>
            </h3>
            <p>
              O passo mais difícil é sempre o primeiro, e você acaba de
              completá-lo. A maioria das pessoas evita olhar para o que as
              incomoda. O fato de você ter respondido a estas perguntas já te
              coloca em um grupo seleto que decidiu parar de reagir no piloto
              automático e começar a assumir o controle.
            </p>
          ...
          <div className={styles.reflectionQuestions}>
            <h3>
              Antes de ver seu resultado, um convite para uma breve reflexão.
            </h3>
            <p>
              Suas respostas aqui são confidenciais e me ajudarão a entender
              ainda melhor o seu momento.
            </p>
            <div className={styles.formGroup}>
              <label htmlFor="reflection1">
                1. Como foi, para você, parar e refletir sobre estas situações
                ao responder o teste?*
              </label>
              <textarea
                id="reflection1"
                rows="3"
            ...
            <div className={styles.formGroup}>
              <label htmlFor="reflection2">
                2. Se você pudesse dar um nome ou usar uma metáfora para
                descrever o que você sente no exato momento em que a raiva
                assume o controle, qual seria? (Ex: "Uma onda que me arrasta",
                "Um curto-circuito na mente").*
              </label>
              <textarea
                id="reflection2"
                rows="3"
            ...
            <div className={styles.formGroup}>
              <label htmlFor="reflection3">
                3. Ao buscar entender seu gatilho com este teste, qual é a
                principal mudança que você espera ver em sua vida?*
              </label>
              <textarea
                id="reflection3"
                rows="3"
            ...
            <div className={styles.formGroup}>
              <label htmlFor="reflection-scale">
                Por fim, em uma escala de 0 a 10, o quanto esta pesquisa te
                ajudou a refletir sobre suas próprias reações?*
              </label>
              <div className={styles.sliderGroup}>
                <input
                  ...
                  max="10"
                  defaultValue="5"
                  className={styles.reflectionSlider}
                  id="reflection-scale"
                />
                <span className={styles.sliderValuespan}>5</span>
              </div>
            </div>
          </div>
          <button id="goto-step3-btn" className="primary-btn">
            Ver Meu Resultado Preliminar
          </button>
        </div>
      </section>

      <section id="step3" className={`step ${styles.testSection}`}>
        <div className={styles.testContainer}>
          ...
          <div className={styles.emailForm}>
            ...
            <p>
              Para receber a análise detalhada e o plano de ação no seu e-mail,
